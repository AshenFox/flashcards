...
export const createFiltersSlice =
  <Filters extends DefaultFilters>({
    defaultFilters,
    queryKey,
  }: CreateFiltersSliceOptions<Filters>): Slice<FilterStore<Filters>> =>
  setAction => {
    ...
    return {
      ...
      setFilter: (filter, value) =>
        set(state => {
          (state.filters as Record<string, FilterValue>)[filter as string] =
            value;
        }, "setFilter"),
...
export const createModulesFilterSlice = ({
  queryKey,
}: {
  queryKey: QueryKey<ModulesFilters>;
}) =>
  createFiltersSlice({
...
export const createCardsFilterSlice = ({
  queryKey,
}: {
  queryKey: QueryKey<CardsFilters>;
}) =>
  createFiltersSlice({
...
export const createModuleCardsFilterSlice = ({
  queryKey,
}: {
  queryKey: QueryKey<ModuleCardsFilters>;
}) =>
  createFiltersSlice({
